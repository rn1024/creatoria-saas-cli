...
import { DocumentFormat, IApiDocumentation } from './interfaces/doc.interface';

@Injectable()
export class DocumentationService implements OnModuleInit {
  private documentation: IApiDocumentation | null = null;
  private docsPath: string;

  constructor(
    private readonly apiDocGenerator: ApiDocGenerator,
    private readonly swaggerConfig: SwaggerConfigService,
    @Inject('DOCUMENTATION_OPTIONS') private readonly options: any,
  ) {
  ...
  async generateApiDocumentation(): Promise<void> {
    ...
    // 保存为不同格式
    await this.exportDocumentation(DocumentFormat.JSON);
    await this.exportDocumentation(DocumentFormat.YAML);
    await this.exportDocumentation(DocumentFormat.HTML);
    await this.exportDocumentation(DocumentFormat.MARKDOWN);
  }

  /**
   * 导出文档
   */
  async exportDocumentation(format: DocumentFormat): Promise<string> {
    if (!this.documentation) {
      this.documentation = await this.apiDocGenerator.generate() as any;
    }
...
<body>
   ...
   * 生成端点导航
   */
  private generateEndpointNav(): string {
   ...
   * 生成端点文档
   */
  private generateEndpointDocs(): string {
    if (!this.documentation?.endpoints) {
      return '';
    }
   ...
   * 生成参数文档
   */
  private generateParametersDocs(parameters?: any[]): string {
   ...
   * 生成响应文档
   */
  private generateResponsesDocs(responses?: any[]): string {
...
`;
   ...
   * 生成配置参考
   */
  private async generateConfigReference(): Promise<void> {
